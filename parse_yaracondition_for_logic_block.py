...
def parse_yaracondition_for_logic_block(parse_target):
    ...
    tokens = ['LPAREN', 'RPAREN', 'COLON', 'NEQUALS'] +  list(reserved.values()) + ['ID', 'STRINGNAME', 'STRINGNAME_COUNT', 'STRINGNAME_ARRAY', 'STRINGNAME_LENGTH'] 
    ...
    def p_ids_double(p):
        ...
        p[0] = [re.sub(r'(@|!)([a-zA-Z0-9]+) \[', r'\1\2[', p[1][0] + ' ' + p[2][0])]
...
def test():
    ...
    assert result1[1] == ['(', '(', '$abcdefg', 'and', '$b', ')', 'or', '(', '$c', 'and', '$d', ')', ')', 'and', 'not', '( $e )']
    
    # case 2
    case2 = '( $a in ( 1 .. 100 ) or ( #b * 2 ) != 6 or ( $c > 100 and !d[1] == 2 ) )'
    result2 = parse_yaracondition_for_logic_block(case2)
    assert result2[0] == ['$a', '#b', '$c', '!d']
    assert result2[1] == ['(', '$a in ( 1 .. 100 )', 'or', '( #b * 2 ) != 6', 'or', '(', '$c > 100', 'and', '!d[1] == 2', ')', ')']
    ...
    # case 4
    case4 = '( ( for any of ( $a, $b, $c ) : ( $ ) and ( $d ) ) or ( for all of ( $g* ) : ( @ > @h ) ) )'
    result4 = parse_yaracondition_for_logic_block(case4)
    assert result4[0] == ['$a', '$b', '$c', '$', '$d', '$g*', '@', '@h']
    assert result4[1] == ['(', '(', 'for', 'any of ( $a , $b , $c )', ':', '( $ )', 'and', '( $d )', ')', 'or', '(', 'for', 'all of ( $g* )', ':', '( @ > @h )', ')', ')']
