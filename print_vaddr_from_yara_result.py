        ...
        for section in self.analyzeobj.sections:
            if section.contains_offset(offset):
                return (
                    ...
                    self.analyzeobj.OPTIONAL_HEADER.ImageBase +
                    section.get_rva_from_offset(offset)
                    )
        else:
            return ('', -1)
    ...
    def get_sectionname_and_virtualaddress(self, offset):
        for section in self.analyzeobj.iter_sections():
            ...
            if offset >= secoffset and offset < secoffset + secsize:
                return (
                    section.name,
                    (offset - secoffset) + section['sh_addr']
                    )
        else:
            return ('', -1)
...
def main(filename, rulefilename=None, allrule=None):
    ...
    if analyzeobj is None:
        fd.close()
        return None

    rulefilenames = []
    if allrule:
        for rulefilename in glob.glob(
                os.path.join(
                    os.path.dirname(
    ...
    resultdict = {}
    # Note: one by one processing is slow
    # (yara matching uses Aho-Corasick algorithm)
    for rulefilename in rulefilenames:
        ...
        for yararesultele in yararesult:
            ...
            for yararesultelestringele in yararesultele.strings:
                secname, vaddr = analyzeobj.get_sectionname_and_virtualaddress(
                        yararesultelestringele[0]
                    )
                result_each_rule_each_string.append({
                        "offset": yararesultelestringele[0],
                        "string_identifier": yararesultelestringele[1],
                        "string_data": yararesultelestringele[2],
                        "section_name": secname,
                        "vaddr": vaddr
                    })
            result_each_rule[(
                    yararesultele.rule,
                    yararesultele.meta['description']
                )] = result_each_rule_each_string
        if result_each_rule != {}:
        ...
        for rule, string in result_each_rule.items():
            ...
            for each_string in string:
                if len(each_string["string_data"]) <= 10:
                    stringhex = ' '.join([
                        hex(c)[2:].zfill(2)
                        for c in each_string["string_data"]
                    ])
                else:
                    stringhex = ' '.join([
                        hex(c)[2:].zfill(2)
                        for c in each_string["string_data"][:10]
                        ]) + ' ...'
                if each_string["vaddr"] > 0:
                    print(
                        '\t\toffset=' +
                        f'{hex(each_string["offset"])}' +
                        '(' +
                        f'vaddr={hex(each_string["vaddr"])}, ' +
                        f'section={each_string["section_name"]}' +
                        '):' +
