...
from parse_yaracondition_for_logic_block \
    import parse_yaracondition_for_logic_block,\
    cleanup as parse_yaracondition_for_logic_block_cleanup
...
class ReferenceRootedGraphNodeManager(object):
    """
    rooted undirected graph whose children is
    referenced function/data from parent
    """
    ...
    def addNode(self, node):
        if type(node) != ReferenceRootedGraphNode:
            raise ValueError(
                "node should be ReferenceRootedGraphNode instance"
                )
        self.nodes[node.offset] = node

    def addChildLink(self, parentnodeoffset, childnodeoffset):
        if not (parentnodeoffset in self.nodes) \
                or not (childnodeoffset in self.nodes):
            raise ValueError(
                "node should be in ReferenceRootedGraphNodeManager"
                )
...
def get_node(refaddr, reftype, r2_bin):
    if reftype is ReferenceType.FUNCTION:
        ...
        # TODO: data size is not specified.
        #       more analysis is required for specifying data size.
        size = 1024
    else:
        raise ValueError("the reference type is invalid at get_node")
    bytecode = base64.b64decode(
        r2_bin.cmd(
                's %s;p6e %d;' % (hex(refaddr), size)
            ).encode()
        )
...
def get_reference(refaddr, reftype, r2_bin):
    if reftype is ReferenceType.FUNCTION:
        ...
        try:
            ...
            r2_bin.quit()
            raise ValueError(
                "cannot extract function info at %s" % hex(refaddr)
                )
        if "callrefs" in refresult:
            callrefs = [
                ele["addr"] for ele in refresult["callrefs"]
                if ele["type"] == "CALL" and
                check_function_exist(ele["addr"], r2_bin)
                ]
        else:
            callrefs = []
        if "datarefs" in refresult:
            ...
            datarefs = []
        return [(ele, ReferenceType.FUNCTION) for ele in callrefs] +\
            [(ele, ReferenceType.DATA) for ele in datarefs]
    else:
        return []
...
def create_reference_graph(refaddr, reftype, depth, r2_bin, curdepth=0):
    ...
    if curdepth < depth:
        for refrefaddr, refreftype in get_reference(refaddr, reftype, r2_bin):
            rrgnm_child = create_reference_graph(
                refrefaddr, refreftype, depth, r2_bin, curdepth+1
                )
...
def targetbyte_refgraph(refgraph):
    nodes = sorted(
            [node for node in refgraph.nodes.values()],
    ...
    def offsetmap(string_offset):
        # search a node which matched string_offset and
        # return vaddr corresponding to string_offset
        # based on offset of the node
        for i in range(len(stringoffsetmap)):
            if string_offset < stringoffsetmap[i]:
                ...
                return (
                        (nodes[i-1].offset + string_offset_in_node),
                        nodes[i-1].offset
                        )
...
def rebuild_rule_refgraph(parsed_rule, stringoffsetmap):
    ...
    For example:
    #a==6 or $b ->
        (#a==6 and (($a in (0..100)) and ($a in (101..200)))) or
        ($b and (($b in (0..100)) and ($b in (101..200))))
    any of them ->
        for any of them : ( $ ) and (($ in (0..100)) or ($ in (101..200)))
    for any of them : ( $ ) ->
        for any of them : ( $ ) and (($ in (0..100)) or ($ in (101..200)))

    NOTE:
        we cannot transform correctly (that is, it is not meaningful) for
        the expression like "any of ($a*) in (1000..2000)" or
        "$a at 100" or "for all i in (1,2,3) : ( @a[i] + 10 == @b[i] )".
    """

    parsed_rule_modified = dict(parsed_rule)
    rule_condition = parsed_rule_modified['condition_terms']

    # reconstruct parsed_rule (for, (, ), :, and, or, not is separated) and
    # compute variables($a, #b, @abc, !d, etc.)
    condition_vars, condition_blocks = parse_yaracondition_for_logic_block(
        ' '.join(rule_condition)
        )
    ...
    def transform_condition_block(condition_block, condition_var_each):
        # transform such as: @a[0]<100 -> ( @a[0]<100 ) and (($a in (0..100))
        # or ($a in (101..200)))
        ...
        for condition_var in condition_var_each:
            ...
            for i in range(0, len(stringoffsetmap)-1):
                ...
                rule_condition_each_var += (
                    ...
                    ('$' + condition_var[1:]) +
                    ' in ( ' +
                    str(stringoffsetmap[i]) +
                    ' .. ' +
                    str(stringoffsetmap[i+1]-1) +
                    ' ) )'
                    ).split(' ')
            transformed_condition += \
                ['and', '('] + rule_condition_each_var + [')']
    ...
    for condition_block in condition_blocks:
        if condition_block == 'for':
            ...
            # transform such as:
            # any of them ->
            #   for any of them: ( $ ) and (($ in (0..100)) and
            #   ($ in (101..200)))
            if not (for_scope):
                rule_condition_modified += ['for'] + [condition_block] +\
                    [':'] + transform_condition_block('( $ )', ['$'])
            else:
                rule_condition_modified += [condition_block]
            ...
            # transform such as:
            # #a==6 ->
            #   (#a==6) and (($a in (0..100)) or ($a in (101..200)))
            condition_var_each = []
            while (True):
                condition_var = condition_vars[condition_vars_pos]
                if condition_var is None or \
                        not (condition_var in condition_block):
                    break
                condition_var_each.append(condition_var)
                condition_vars_pos += 1
            rule_condition_modified += transform_condition_block(
                condition_block, condition_var_each
                )
        else:
            rule_condition_modified += [condition_block]
...
def check_yararule_for_referenced_node(refgraph, parsed_rules):
    ...
    return {
        ele.rule: [
            string_convert_to_refaddr_offset(elestr, offsetmap) for elestr
            in ele.strings
            ] for ele in matches
        }
...
def main(
        filename, funcaddr, depth=2,
        rulefilename=None,
        allrule=False, allfunc=False, funcmain=False
        ):
    ...
    # TODO:
    #   to deal with include recursively
    #   (plyara itself does not handle include, just parse it)
    rulefilenames = []
    if allrule:
        for rulefilename in glob.glob(os.path.join(os.path.dirname(
                os.path.abspath(__file__)
                ), 'yararules', '*', '*.yar')):
            rulefilenames.append(rulefilename)
    elif rulefilename is None:
    ...
    try:
        ...
        for rulefilename in rulefilenames:
            ...
            for function_addr in funcaddres:
                refgraph = create_reference_graph(
                    function_addr, ReferenceType.FUNCTION, depth, r2_bin
                    )
                resultrules = check_yararule_for_referenced_node(
                    refgraph, parsed_rules
                    )
            ...
            for parsed_rule in parsed_rules:
                for resultrules in resultrules_func:
                    try:
                        resultrules_parsedrule = resultrules[
                            parsed_rule['rule_name']
                            ]
                    except KeyError:
                        continue
                    rule_description = [
                        ele for ele in parsed_rule['metadata']
                        if type(ele) == dict and
                        'description' in ele
                        ][0]['description']
                    if not (rule_description in resulteachruledict):
                        resulteachruledict[
                            (parsed_rule['rule_name'], rule_description)
                            ] = resultrules_parsedrule
                    else:
                        resulteachruledict[
                            (parsed_rule['rule_name'], rule_description)
                            ] += resultrules_parsedrule
        ...
        for rule, string in result_each_rule.items():
            ...
            for each_string in string:
                if len(each_string["string_data"]) <= 10:
                    stringhex = ' '.join([
                            hex(c)[2:].zfill(2) for c
                            in each_string["string_data"]
                        ])
                else:
                    stringhex = ' '.join([
                            hex(c)[2:].zfill(2) for c
                            in each_string["string_data"][:10]
                        ]) + ' ...'
                print(
                    '\t\tvaddr=' +
                    f'{hex(each_string["vaddr"])}' +
                    f'(nodeaddr={hex(each_string["nodeaddr"])}):' +
                    f'{each_string["string_identifier"]}: {stringhex}'
                    )
...
if __name__ == '__main__':
    parser = argparse.ArgumentParser(
        description="" +
        "yara rule matching within functions/data referenced " +
        "from given function address"
        )

    parser.add_argument(
        "filename",
        help="filename for the binary of analyzing target"
        )
    parser.add_argument(
        "--rulefilename",
        help="filename for the yara file which includes specific yara rules"
        )
    parser.add_argument(
        "--allrule",
        help="flag for processing all defined yara rules one by one",
        action='store_true'
        )
    ...
    group.add_argument(
        "--funcaddr",
        help="function address of starting point for analyzing the binary. " +
        "The address expression should be recognizable by radare2 " +
        "(such as hex string, decimal string, function name, etc.)"
        )
    group.add_argument(
        "--funcmain",
        help="use main function (or entrypoint) for starting point " +
        "for analyzing the binary",
        action='store_true'
        )
    group.add_argument(
        "--allfunc",
        help="flag for analyzing all function " +
        "(much time for large binary)",
        action='store_true'
        )

    parser.add_argument(
        "--depth",
        help="depth for analyzing function call/data reference (default: 2)",
        default=2, type=int
        )
    parser.add_argument(
        "--quiet",
    ...
    result = main(
        args.filename, args.funcaddr, depth=args.depth,
        rulefilename=args.rulefilename,
        allrule=args.allrule, allfunc=args.allfunc, funcmain=args.funcmain
        )
    view(result)
